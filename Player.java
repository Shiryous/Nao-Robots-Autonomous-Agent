...
public abstract class Player extends Robot {
  ...
  private Motion standUpFromFrontMotion, rollFromBackToFrontMotion;
  ...
  public Player(int playerID, int teamID) {
    ...
    rollFromBackToFrontMotion = new Motion("../../motions/RollFromBackToFront.motion");
  }
  ...
  protected void getUpIfNecessary() {
    ...
    double[] accelaration = accelerometer.getValues();
    if (accelaration[0] < -9)
      playMotion(standUpFromFrontMotion);
    else if (accelaration[0] > 9){
      playMotion(rollFromBackToFrontMotion); // ROLL FROM BACK 
      playMotion(standUpFromFrontMotion); // Then get up normally
    }
  }
